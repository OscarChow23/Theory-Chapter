...
Underground Research Facility~\cite{DUNE2025Science}. Matter effects are enhanced due to the longer baseline  
resulting in more effective separation of the $\dcp$ interference by its different
energy dependence (Sec.~\ref{sec:matter}), while the wide-band beam measures both $\nue$
($\nuebar$) appearance and $\numu$ ($\numubar$) disappearance as functions of energy. That
spectral information is what breaks the degeneracies among $\dcp$, the mass ordering and
$\theta_{23}$ that a single $L/E$ cannot~\citep[p.~3]{DUNE2025Science}. A movable near
detector (DUNE-PRISM) characterises neutrino--argon interactions over a range of off-axis
energies, so that the remaining systematic uncertainties can be held at the few-percent
level required for the precision programme~\cite{DUNE2025Science}.

\paragraph{Hyper-Kamiokande.}
Hyper-Kamiokande is a water-Cherenkov detector on T2K's $295$\,km
baseline~\cite{HyperK2025}. There the matter effect is small, so the beam measurement is
of $\dcp$ first; the ordering is taken mainly from the atmospheric sample.

The complementarity is therefore structural rather than incidental: DUNE uses matter to
fix the ordering before reading $\dcp$, while Hyper-Kamiokande measures $\dcp$ where
matter is a small correction. A disagreement between them would be hard to attribute to a
shared systematic.
