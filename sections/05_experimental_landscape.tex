...
Figure~\ref{fig:juno-solar} shows JUNO's $\dm{21}$ vs $\sin^2 \theta_{12}$ confidence intervals
compared with the experiments already discussed, alongside additional results shown in the figure.
Comparing the error bars for each result shows
...
\begin{figure}[tb]
    ...
    profiles; the $\dm{21}$ panel also shows the SNO+ (2025) reactor result.
    Reproduced from the JUNO Collaboration~\citep[Fig.~4, p.~14]{JUNO2026}.}
...
\begin{figure}[tb]
    ...
    Four values of $\dcp$ are labelled for reference, with one ellipse for the normal (blue), and one
...
\paragraph{DUNE.}
The Deep Underground Neutrino Experiment (DUNE) will send a wide-band neutrino beam $1285$\,km from Fermilab to
...
\begin{figure}[tb]
    ...
    different values of $\dcp$. The first oscillation
    maximum is the broad peak near $2.5$\,GeV; the second is the sharper peak
    near $0.8$\,GeV. DUNE's wide-band beam covers both these maxima. Reproduced from the DUNE Collaboration~\citep[Fig.~1, p.~8]{DUNE2020}.}
...
\begin{table}[htbp]
    ...
    Reproduced from the NuFIT~6.1 table~\cite{NuFIT61};
