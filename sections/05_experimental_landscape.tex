...
but with a shorter baseline of $295$\,km and an energy spectrum peaking at a lower at around
$0.6$\,GeV. Both experiments sit near the first disappearance dip and as a result are broadly consistent in their 
...
degeneracy introduced in Section~\ref{sec:matter-effect} manifests differently in both experiments. The bi-event plot
...
period, from $-\pi$ to $\pi$, the predicted number of events traces out an ellipse in this space, with each ordering tracing out its
own ellipse. The degeneracy explained in Section~\ref{sec:matter-effect}, is clearly visible in this plot, that is, the 
appearance rate for the normal ordering hypothesis with $\dcp\simeq+\pi/2$ yields an almost identical rate to the 
inverted ordering hypothesis with $\dcp\simeq-\pi/2$. The ordering pulls the rate in the opposite direction 
to the CP term. With T2K's smaller matter effect, the ordering ellipses are pulled apart by a much smaller amount. 
T2K's ordering sensitivity therefore suffers from higher degenracy as there is more overlap between ellipses, whereas
at their tips, there is less $\dcp$ degeneracy.





Understand NOvA-T2K results as well, finding 
Continue reading about NOvA-T2K joint analysis on the cursor chat 
"Degeneracy in t2k matter effects".


 

The consequence is that the two experiments break the
ordering--$\dcp$ degeneracy differently. Concretely: at NOvA, a normal-ordering
hypothesis with $\dcp\simeq+\pi/2$, which lowers the $\nue$ appearance rate, can
produce the same rates as an inverted-ordering hypothesis with $\dcp\simeq-\pi/2$,
which raises them, because the matter term and the CP interference pull in
opposite directions and can cancel. At T2K the matter term is small, so that
mimicry is unavailable; an excess of $\nue$ over $\nuebar$ is then a nearly pure
$\dcp$ measurement, preferring $-\pi/2$ in either ordering, but it cannot tell
the orderings apart. NOvA therefore constrains the ordering better, T2K
constrains $\dcp$ better, and their appearance data can be combined to hold one
parameter fixed while the other is read off.

Figure~\ref{fig:bievent} shows a bi-event plot this more clearly. Plotting the expected $\nue$ against
...
NOvA and T2K performed a first joint analysis of both their datasets and 
...
accelerator experiments are complimentary in that their measurements should agree under the correct mass 
ordering. Therefore, as the JUNO experiment matures and its $\dm{3\ell}$ measurements become more precise, 
this can be input to NOvA's analysis as a sharper external constraint than the current one provided by 
Daya Bay, potentially strenghtnening the ordering preference~\cite{NOvAJUNO2026}. A more quantitative treatment
of this potential improvement will be 
