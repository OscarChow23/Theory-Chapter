...
The flavour of a neutrino denotes its state when it is produced or detected. It was previously mentioned that neutrinos oscillate, or change flavour as they propagate. A neutrino produced as a given flavour
may therefore be detected as a different flavour at a later point. This means that a propagating neutrino experiences and evolves as a function of time. This statement is not true for massless particles, as they travel at the speed of light, so no proper time elapses as they travel. Neutrino oscillations therefore, imply that neutrinos are massive.

Section~\ref{sec:flavour-mass} introduces the orthogonal bases that neutrino oscillations (or flavour change) require, namely, flavour and mass eigenstates. 
Section~\ref{sec:two-flavour} begins the oscillation probability derivation for the two-flavour case. Section~\ref{sec:pmns} then introduces how these bases are related by a unitary mixing matrix and gives an overview of the oscillation parameters. This allows for the expansion of the theory to three flavours in Sec.~\ref{sec:three-flavour}, where the ordering of the mass states is also discussed\footnote{An open question that this thesis addresses.}. Section~\ref{sec:matter} then discusses an important effect that arises
due to the propagation of neutrinos through the Earth (or more generally,
through matter), which primarily modifies the appearance probability.
...
\end{equation}
$\ket{\nu_\alpha}$ denotes the flavour eigenstates, where $N$ is the number of flavours, and $\ket{\nu_j}$ denotes the mass eigenstates, with $U$ being the unitary mixing matrix, known as the Pontecorvo--Maki--Nakagawa--Sakata (PMNS) matrix, which will be discussed in more detail in Sec.~\ref{sec:pmns}. In the three-flavour picture, $N=3$, and $\alpha \in \{e,\mu,\tau\}$.
Convention is for $U$ to be conjugated in the first relation. For
antineutrinos, $U$ and $U^{*}$ are interchanged, so the conjugate
appears in the second relation instead. This flips the sign of every
imaginary part of $U$.
...
It is useful to begin with with the two flavour derivation, as it is simpler, but already shows that the mixing angle sets the oscillation amplitude and the mass-squared splitting sets the frequency in $L/E$. This fact is the leading behaviour of the three-flavour $\numu$ survival probability, which will be derived later.
To derive the two-flavour oscillation mechanism, we first consider the flavour and mass eigenstate mixing relations of Eq.~\hyperref[eq:mixing-relation]{(\ref{eq:mixing-relation})}. To begin, it is useful first, to consider the picture with two flavour states, $\ket{\nu_\alpha}$ and $\ket{\nu_\beta}$, and two mass states, $\ket{\nu_1}$ and $\ket{\nu_2}$, with masses $m_1$ and $m_2$. The unitary mixing matrix $U$ can be express as a $2\times2$ rotation matrix through an angle $\theta$:
...
Next, we reiterate that the neutrino mass states are eigenstates of the free particle Hamiltonian. Therefore, they evolve as a function of time as plane wave solutions to the time-dependent Schrödinger equation:
\begin{equation}
...
Using the assumption that neutrinos are ultra-relativistic, that is, $m_j \ll E_j$, and by choosing the equal momentum idealisation of the mass states, it can be shown that the energy of the $j$th mass state can be written as:
\begin{equation}
    E_j = \sqrt{|\vec{p}_j|^{2} + m_j^{2}} \simeq |\vec{p}_j| + \frac{m_j^{2}}{2|\vec{p}_j|} \simeq E + \frac{m_j^{2}}{2E},
    \label{eq:relativistic-expansion}
\end{equation}
where $E$ is the common leading order energy of the states. The ultra-relativistic limit allows us to approximate the time as the propagation distance, $t \simeq L$, when working in natural units. The plane wave exponent, or the phase accumulated therefore becomes:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:amplitude}
\end{equation}
By taking the squared modulus of this amplitude, and using the identity $|e^{-i\phi_2}-e^{-i\phi_1}|^{2} = 2 - 2\cos\Delta\phi = 4\sin^{2}(\Delta\phi/2)$, where $\Delta\phi = \phi_2 - \phi_1 = \dm{}L/2E$ with $\dm{} \equiv m_2^{2}-m_1^{2}$, the appearance-channel transition probability becomes:
\begin{equation}
    P(\nu_\alpha \to \nu_\beta)
...
\end{equation}
One can also write the disappearance-channel survival probability as the complement, as we are only considering two flavours,
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \caption{Panels showing the two-flavour survival probability as a function of propagation distance at fixed
    energy (\textbf{left}) and of neutrino energy at fixed baseline (\textbf{right}). The mixing
    angle sets the amplitude of the oscillation through $\sin^{2}(2\theta)$, while $\dm{}$ determines the frequency by setting the oscillation phase. The variation as a function of energy is not periodic. This is due to the $1/E$ dependence of the phase. The highest-energy minimum lies at $E_{\rm max}$.
    Generated from Eq.~\ref{eq:two-flavour-practical}.
    The layout is adapted from Fig.~2.4 of Ref.~\cite{Booth2021}, which is itself taken from Ref.~\cite{Hernandez2016}.}
...
\begin{equation}
    ...
    \label{eq:first-max}
\end{equation}
Maximum disappearance occurs when $\sin^2x = 1$, i.e. when $1.27\dm{}L/E = \pi/2$. Equation~\hyperref[eq:first-max]{(\ref{eq:first-max})} follows after rearranging.

The full picture as we know it however, is more complex. The single angle $\theta$ and single splitting $\dm{}$ used above are a simplification of the three-flavour theory. The three-flavour picture replaces the $2\times2$ unitary rotation matrix of Eq.~\hyperref[eq:two-flavour-mixing-matrix]{(\ref{eq:two-flavour-mixing-matrix})} with the more complete $3\times3$ PMNS mixing matrix. The following sections will examine its structure and parameterisations, and will expand the derivation presented here to three flavours in both the appearance and disappearance channels.
...
Combining the three-flavour equivalent expressions of Eq.~\hyperref[eq:two-flavour-mixing]{(\ref{eq:two-flavour-mixing})}, Eq.~\hyperref[eq:plane-wave-solution]{(\ref{eq:plane-wave-solution})} and Eq.~\hyperref[eq:phase]{(\ref{eq:phase})}, one can write the general three-flavour probability as:
\begin{align}
...
\end{align}
where $\delta_{\alpha\beta}$ is the Kronecker delta. For flavour transition probabilities, that is, when $\alpha \neq \beta$, the sign of the imaginary part is flipped in the antineutrino case. Therefore, for oscillations in vacuum, this asymmetry is a potential probe into CP violation in the lepton sector.
...
The product $U^{*}_{\alpha i}U_{\alpha i}U_{\alpha j}U^{*}_{\alpha j} = |U_{\alpha i}|^{2}|U_{\alpha j}|^{2}$, is real by definition, so there exists no imaginary term to introduce a sign change when considering antineutrinos.
...
than $\Delta_{31}$ and $\Delta_{32}$~\cite{NunokawaParkeValle2008,PDG2024}. As a result,
$\dm{21} \ll |\dm{32}|$, which renders $\dm{31} \simeq \dm{32}$ and
$\sin^{2}{\Delta_{21}} \simeq 0$. Using the unitarity relation, $|U_{\mu 1}|^{2} + |U_{\mu 2}|^{2} = 1 - |U_{\mu 3}|^{2}$, the survival probability can then be approximated as:
\begin{equation}
    P(\numu \to \numu) \simeq 1 - 4|U_{\mu 3}|^{2}\left(1 - |U_{\mu 3}|^{2}\right)\sin^{2}\Delta_{31}.
    \label{eq:pmumu-effective}
\end{equation}
This mirrors the two-flavour expression of Eq.~\hyperref[eq:two-flavour-survival]{(\ref{eq:two-flavour-survival})} with the amplitude $4|U_{\mu3}|^{2}(1-|U_{\mu3}|^{2})$ replaces $\sin^{2}(2\theta)$. It is clear therefore that the disappearance channel informs on $|U_{\mu 3}|^{2}$, i.e. the fraction of the muon flavour carried by the third mass state, as introduced previously in Sec.~\ref{sec:pmns}.
...
It is therefore clear from this expression that in analogy to Fig.~\ref{fig:two-flavour-survival-probability}, the amplitude of the oscillations is directly related to the mixing angle $\theta_{23}$ and the position of the dip informs the value of $\dm{32}$.
...
The leading term expression for the survival probability is symmetric about $\theta_{23} = \pi/4$, since the amplitude $\sin^{2}2\theta_{23}$ is invariant under the transformation $\theta_{23}\rightarrow \pi/2 - \theta_{23}$. The disappearance channel is therefore not sensitive to whether $\theta_{23}$ lies above or below $\pi/4$ (the maximal mixing case). This octant degeneracy is broken by the $\cos 2\theta_{23}$ term in the correction term of Eq.~\hyperref[eq:pmumu-expanded]{(\ref{eq:pmumu-expanded})}, but for NOvA, the disappearance channel alone leads to only a weak preference for the octant due to the strength of the leading term.

\subsubsection*{Neutrino mass ordering}
The general form of oscillation probabilities of Eq.~\hyperref[eq:three-flavour-general]{(\ref{eq:three-flavour-general})} have dependence on the square of the mass-squared splittings $\dm{32}$. Therefore, the sign of $\dm{32}$ may not be known.
The sign of $\dm{21}$ however, is known to be positive from solar neutrinos (see Sec.~\ref{sec:landscape-solar-reactor}). 
This is known as the neutrino mass ordering problem. That is, it is possible for the third neutrino mass state to lie above or below $\nu_1$ and $\nu_2$.
The two possibilities are known as the \emph{normal ordering}, where $m_1 < m_2 < m_3$, and the \emph{inverted ordering}, where $m_3 < m_1 < m_2$. These are shown in the diagram of Fig.~\ref{fig:mass-ordering}.
\begin{figure}[htbp]
    \centering
    ...
    $\dm{21}$, which is known to be positive from solar neutrinos, is much smaller than the atmospheric splitting
    $|\dm{32}|$. The shaded colours indicate the approximate flavour
    composition $|U_{\alpha j}|^{2}$ of each mass state.
    Adapted from Fig.~2.6 of Ref.~\cite{Booth2021}.}
...
As mentioned, a $\numu$ flavour state is a superposition of mass eigenstates, that each propagate with their own phase. For appearing $\nue$s, these are then projected onto the $\nue$ state when detected.
...
Since $\nu_1$ and $\nu_2$ are separated by a small $\dm{21}\sim 10^{-5}\,\mathrm{eV}^{2}$, while $\nu_3$ is separated by a much larger $|\dm{31}|\sim 10^{-3}\,\mathrm{eV}^{2}$, it is useful to group the three terms into two amplitudes: an atmospheric amplitude, most closely associated with the fast oscillations of the $j=3$ term, and a so-called solar amplitude, in which the slow solar phase between $\nu_1$ and $\nu_2$ allows those two terms to be grouped together.              
...
Furthermore, in addition, $\theta_{23}$ enters the leading order $P_{atm}$ term via $\sin^2\theta_{23}$, in contrast to $\sin^22\theta_{23}$ in the disappearance case. The symmetry about $\pi /4$ is no longer present, which suggests that vacuum appearance data can, unlike $\numu$ disappearance can resolve the octant of $\theta_{23}$. These are vacuum expressions. The effect of neutrinos travelling through matter is particularly important for the appearance channel. The following subsection touches on how neutrino propagation through matter changes the oscillation probability.
...
However, since matter consists almost entirely of electrons, rather than $\mu$ or $\tau$ leptons, $\nue$s have access to additional charged-current scattering interactions. As they propagate, they accumulate an additional relative phase, which is observable \cite{Wolfenstein1978,Mikheyev1985,Mikola2023}.
Figure~\ref{fig:matter-scattering} shows the CC processes unique to the electron flavour (left and middle), while the NC process (right) occurs with equal probability for all three flavours.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{booth_matter_scattering.pdf}
    \caption{Charged-current (left and middle) and neutral-current (right) coherent
    scattering of neutrinos or antineutrinos on electrons. The NC channel occurs
    equally for all neutrino flavours, whereas CC scattering is unique to the
    electron flavour. Reproduced from Fig.~2.5 of Ref.~\cite{Booth2021},
    originally from Ref.~\cite{Mendez2019}.}
    \label{fig:matter-scattering}
\end{figure}
...
Considering the first oscillation maximum of Fig.~\ref{fig:nova-matter-probability}, the shaded regions show that varying the value of $\dcp$ can shift the appearance probability in both directions. As a consequence, a normal ordering hypothesis with a $\dcp$ value that lowers the appearance probability can be consistent with an inverted ordering hypothesis with a different $\dcp$ value that enhances the appearance probability. This leads to a degeneracy between the mass ordering and $\dcp$. Measuring an appearance probability near the vacuum curve or the shaded overlap region does not allow either the mass ordering or $\dcp$ to be uniquely determined. The addition of antineutrino appearance data would help to resolve this degeneracy. This is due to both the matter potential, $V_{\rm CC}$, and the CP violating term, $\dcp$ changing sign relative to the neutrino case. Combinations of mass ordering and $\dcp$ values that give the same neutrino appearance probability will give different probabilities for the antineutrino case.
